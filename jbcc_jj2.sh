...
(
  ...
  local params=()
  local source_json_path=/Users/hiyoko/code/JsonBasedCommandCompletion/jj2.json
...
jj2() {
  ...
  read source_json_path static_path params <<< $(_make_path_jj2 "$@")
  if [[ -z $source_json_path ]]; then
    echo "Error: no path is defined at [$@] in \"/Users/hiyoko/code/JsonBasedCommandCompletion/jj2.json\""
    return 1;
  fi
...
__completion_jj2()
{
  ...
  local static_path params source_json_path
  read source_json_path static_path params <<< $(_make_path_jj2 ${COMP_WORDS[@]:1:(COMP_CWORD-1)})
...
complete -F __completion_jj2 jj2
echo "complete -F __completion_jj2 jj2...."
