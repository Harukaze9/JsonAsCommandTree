# jj and jj will be replaced when creating temporary scripts.
...
(
  ...
  local params=()
  local source_json_path=/Users/hiyoko/code/JsonBasedCommandCompletion/commands/jj.json
...
jj() {
  ...
  read source_json_path static_path params <<< $(_make_path_jj "$@")
  if [[ -z $source_json_path ]]; then
    echo "JBCC Error: no path is defined at [$@] in \"/Users/hiyoko/code/JsonBasedCommandCompletion/commands/jj.json\""
    return 1;
  fi
...
__completion_jj()
{
  ...
  local static_path params source_json_path
  read source_json_path static_path params <<< $(_make_path_jj ${COMP_WORDS[@]:1:(COMP_CWORD-1)})
...
complete -F __completion_jj jj
# echo "complete -F __completion_jj jj...."
